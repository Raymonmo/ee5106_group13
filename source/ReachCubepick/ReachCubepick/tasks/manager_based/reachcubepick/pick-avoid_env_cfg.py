...
class ReachcubepickSceneCfg(InteractiveSceneCfg):
    ...
    wall = RigidObjectCfg(
        ...
        spawn=CuboidCfg(
            size=(0.02, 0.3, 0.15),
            # 保持 kinematic_enabled=True，确保它不会掉落或被撞飞
            rigid_props=sim_utils.RigidBodyPropertiesCfg(kinematic_enabled=True), 
            collision_props=sim_utils.CollisionPropertiesCfg(),
            visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(1.0, 0.0, 0.0)),
        ),
        # 注意：这里要用 RigidObjectCfg.InitialStateCfg
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.25, 0.0, 0.075))
    )
    ...
    contact_forces = ContactSensorCfg(
        ...
        # [新增过滤] 传感器只记录与 "地面" 和 "墙" 的碰撞
        # 这样机器人碰到 Cube 不会死，但碰到墙或地会死。
        # filter_prim_paths_expr=["{ENV_REGEX_NS}/World/ground", "{ENV_REGEX_NS}/Wall"]
    )
...
class EventCfg:
    ...
    reset_wall_pos = EventTerm(
        func=mdp.reset_root_state_uniform,
        mode="reset",
        params={
            # 让墙在 x 轴 [0.2, 0.3] 之间随机移动
            "pose_range": {"x": (0.2, 0.3), "y": (-0.1, 0.1), "z": (0.075, 0.075)}, 
            "velocity_range": {},
            "asset_cfg": SceneEntityCfg("wall"), # 对应 Scene 里的名字
        },
    )
